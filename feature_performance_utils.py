...
def save_roc_curve(clf, X_test, y_test, roc_figure_save = True, n_classes = 2):
    ...
    if roc_figure_save : plt.savefig('roc_curve.jpg')
    # plt.show()
    plt.close()
...
#     y_test, y_pred = normal_porosis(y_test, y_pred)
    ...
    test_auc_score, best_tr = save_roc_curve(clf=clf, X_test=X_test, y_test=y_test, roc_figure_save=True, n_classes=2)
